...
@doc """
  ...
  corresponding HTML tags. This program is run 
  """

  defmodule ResaltadorSintaxis do

    def lexer_parallel() do
  ...
  #Function that calculates the time it takes to syntax highlight all the files sequentially
  def seq_hl() do
    {time, result} = :timer.tc(fn -> Hw.Primes.lexer() end)
    IO.puts("Time when sequential: #{time}")
    IO.puts("Sequential result: #{result}")
    time
  end

  # Function that calculates the time it takes to syntax highlight all the files in parallel
  def par_hl(cores) do
    {time, result} = :timer.tc(fn -> Hw.Primes.lexer_parallel() end)
